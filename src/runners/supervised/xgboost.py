...
from src.models.supervised.xgboost import predict_xgboost, train_xgboost
...
class XGBoost:
    ...
    def test(self) -> None:
        try:
            model_sma = joblib.load('src/models/saved/xgboost/xgboost_SMA.pkl')
        except FileNotFoundError:
            model_sma = train_xgboost(self.sma_x_train, self.sma_y_train, frequency="SMA")
            
        try:
            model_fm = joblib.load('src/models/saved/xgboost/xgboost_FM.pkl')
        except FileNotFoundError:
            model_fm = train_xgboost(self.fm_x_train, self.fm_y_train, frequency="FM")

        pred_sma = predict_xgboost(model_sma, self.sma_x_test)
        pred_fm = predict_xgboost(model_fm, self.fm_x_test)
        ...
        self.report.generate_supervised_report('xgboost', 'SMA', results_sma)
        self.report.generate_supervised_report('xgboost', 'FM', results_fm)

    def train(self) -> None:
        train_xgboost(self.sma_x_train, self.sma_y_train, frequency="SMA")
        train_xgboost(self.fm_x_train, self.fm_y_train, frequency="FM")
